...
function PawnFindScaleTemplate(ClassID, SpecID)
	...
	-- CoA fix: on this server NONE of the 13 vanilla/retail classes these templates are keyed to
	-- exist -- every one of the 21 custom classes will ALWAYS land here, every time, as the
	-- normal/expected case (not a rare bug). VgerCore.Fail would print a loud, alarming error to
	-- every single player on this server the first time Pawn tries to suggest an automatic/default
	-- scale for their class. Fail quietly instead -- callers already handle a nil return
	-- (PawnGetStatValuesForTemplate returns an empty stat table for it).
	if PawnCommon and PawnCommon.Debug then
		PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnFindScaleTemplate: no built-in template for class " .. tostring(ClassID) .. "/spec " .. tostring(SpecID) .. " (expected for a custom class on this server)")
	end
	return nil
end
...
function PawnGetStatValuesForTemplate(Template, NoStats)
	...
	-- CoA fix: custom classes (SunCleric and others) don't have a template mapped in
	-- PawnScaleTemplates/PawnScaleTemplatesClassic, so PawnFindScaleTemplate returns nil. Without
	-- this guard, the code below (Template.ClassID) would throw a Lua error every time Pawn tried
	-- to build an automatic default scale for one of these classes (e.g. in the quest reward
	-- window, or "Automatic" mode).
	...
	-- CoA fix: mesma logica do CanEquip em Pawn.lua -- se a classe nao tiver entrada em
	-- PawnNeverUsableStats (classe custom), simplesmente nao restringe nada aqui.
	if PawnNeverUsableStats[Template.ClassID] then
