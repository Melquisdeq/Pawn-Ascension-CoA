...
PawnSeparators =
{
	...
	"\r\n", -- CoA fix: random-suffix items ("of the Witch Doctor" etc.) sometimes put several
	        -- stats on a single tooltip line separated by \r\n, instead of one line per stat like
	        -- the normal Blizzard client does.
}
...
PawnRegexes =
{
	...
	{"^Equip: Increases your armor penetration rating by (-?[%d%., ]+)%.$", "ArmorPenetration"}, -- CoA fix: combines "your" + "rating" (Dunemaul Crusher, item 515838)
	...
	{"^Equip: Increases spell penetration by (%-?[%d%., ]+)%.$", "SpellPenetration"}, -- CoA fix: server text without "your" (Rune of Perfection, item 212347)
	...
	{"^%+?(-?[%d%., ]+) [Hh]ealth per 5 sec%.?$", "Hp5"}, -- CoA fix: random-suffix items show this as a direct stat (without "Equip: Restores...")
	{"^%+?(-?[%d%., ]+) [Hh]ealth every 5 sec%.?$", "Hp5"}, -- CoA fix: same thing, "every" variant
