...
-- Polyfill: QuestInfo_GetRewardButton doesn't exist on the 3.3.5a client (only added in later
-- expansions). On 3.3.5, the reward buttons are fixed global frames: QuestInfoItem1, QuestInfoItem2, etc.
if not QuestInfo_GetRewardButton then
	function QuestInfo_GetRewardButton(rewardsFrame, index)
		return _G["QuestInfoItem" .. index]
	end
end

-- Polyfill: PawnCanItemBeArtifactUpgrade doesn't exist on the 3.3.5a client -- the artifact weapon
-- relic system is from Legion and doesn't exist on this client, so no item can be that kind of upgrade.
if not PawnCanItemBeArtifactUpgrade then
