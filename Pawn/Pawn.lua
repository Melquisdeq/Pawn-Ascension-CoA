...
-- CoA fix: safety net. CONFIRMED via diagnostics (a raw log of every event PawnOnEvent receives)
-- that the Ascension CoA server simply never fires UNIT_INVENTORY_CHANGED to the client for these
-- equip actions -- other events (ITEM_LOCKED, ITEM_UNLOCKED) arrive normally, but this one never
-- does, in any of many tests. Since the server can't be fixed from here, we stop relying solely on
-- that event: PawnCoARescanAllEquippedSlots() is also called from ITEM_LOCKED/ITEM_UNLOCKED below,
-- plus this periodic poll as an ultimate fallback for any equip path that doesn't trigger those
-- either (e.g. some macro/addon).
-- The poll cost is genuinely tiny even every tick: PawnCoARescanAllEquippedSlots does at most 19
-- cheap native GetInventoryItemLink lookups plus a light regex match per already-cached slot, and
-- it does nothing at all (no GameTooltip access, no log) when nothing changed -- so a short
-- interval here is not a real performance concern, and keeps the "how long until the bag tooltip
-- shows the upgrade%%" delay imperceptible.
PawnCoAEquipPollDelay = 0
local PAWN_COA_EQUIP_POLL_INTERVAL = 0.2
function PawnCoAStartEquipPolling()
	if PawnCoAEquipPollFrame then return end
	PawnCoAEquipPollFrame = CreateFrame("Frame")
	PawnCoAEquipPollFrame:SetScript("OnUpdate", function(self, elapsed)
		PawnCoAEquipPollDelay = PawnCoAEquipPollDelay + elapsed
		if PawnCoAEquipPollDelay < PAWN_COA_EQUIP_POLL_INTERVAL then return end
		PawnCoAEquipPollDelay = 0
		PawnCoATryRescanAllEquippedSlots()
	end)
end

-- CoA fix: small helper so every call site (event handlers, poll, login) shares the same
-- error-isolation logic instead of repeating the pcall boilerplate several times.
function PawnCoATryRescanAllEquippedSlots()
	local Ok, Err = pcall(PawnCoARescanAllEquippedSlots)
	if not Ok and PawnCommon.Debug then
		PawnDebugMessage(VgerCore.Color.Red .. "[CoA] PawnCoARescanAllEquippedSlots error: " .. tostring(Err))
	end
end
...
function PawnOnEvent(Event, arg1, arg2, ...)
	if Event == "UNIT_INVENTORY_CHANGED" and arg1 == "player" then
		-- CoA fix: kept for the (currently unconfirmed) case where this event does fire on some
		-- action -- see the ITEM_LOCKED/ITEM_UNLOCKED fast paths and the poll above for why this
		-- can't be the only trigger on this server.
		PawnCoATryRescanAllEquippedSlots()
		PawnOnInventoryChanged()
	elseif Event == "ITEM_LOCKED" then
		PawnOnItemLocked(arg1, arg2)
		PawnOnInventoryChanged()
		-- CoA fix: ITEM_LOCKED fires at the START of an item interaction (pickup/drag/right-click),
		-- before the equip has necessarily finished processing server-side -- so a rescan triggered
		-- here can sometimes still see the old gear. Still worth trying (it's free), but
		-- ITEM_UNLOCKED below and the short poll above are what actually guarantee catching the
		-- change quickly.
		PawnCoATryRescanAllEquippedSlots()
	elseif Event == "ITEM_UNLOCKED" then
		-- CoA fix: fires when the item's in-transit lock clears -- i.e. right after the operation
		-- (equip, move, etc.) has actually completed. This is a much more reliable "it's done now"
		-- signal than ITEM_LOCKED, and combined with the short poll above is what makes the
		-- upgrade%% comparison update quickly after equipping instead of waiting for a fixed delay.
		PawnCoATryRescanAllEquippedSlots()
	...
	elseif Event == "PLAYER_LOGIN" then
		PawnInitialize()
		-- CoA fix: make sure the equipped-slot cache starts populated at login, and start the
		-- safety-net poll for the rest of the session.
		PawnCoARescanAllEquippedSlots()
		PawnCoAStartEquipPolling()
...
function PawnInitialize()
	...
	-- CoA fix: PawnUI_GroupLootFrame_OnShow (and the PawnUI_LootUpgradeAdvisor_OnLoad/_OnClick
	-- functions the XML glow-box template expects) are referenced but were never actually defined
	-- anywhere in this file -- same as the bag/quest upgrade advisor blocks right above, which the
	-- original author left commented out. Left active, this risks a Lua error every time
	-- PawnInitialize runs (at login), which would silently abort the rest of that function.
	-- Disabling this until/unless the visual glow-box advisor feature is actually implemented --
	-- the loot/roll TOOLTIP upgrade comparison (hovering the item) does not depend on this and
	-- keeps working normally; see the ReadFromTooltip fix above for that.
...
-- CoA fix: in-memory buffer with the full debug message history, immune to the game chat's
-- scrollback limit. The chat window was cutting off exactly the START of debug bursts (e.g. right
-- at the moment of an event like UNIT_INVENTORY_CHANGED), which could lead to wrongly concluding
-- something never fired when it had actually just scrolled off screen before being copied. See
-- PawnCoAShowDebugLog / "/pawn debuglog".
PawnCoADebugLogBuffer = PawnCoADebugLogBuffer or {}
...
function PawnDebugMessage(Message)
	if PawnCommon.Debug then
		...
		table.insert(PawnCoADebugLogBuffer, date("%H:%M:%S") .. "  " .. tostring(Message))
		if #PawnCoADebugLogBuffer > 3000 then
			table.remove(PawnCoADebugLogBuffer, 1)
		end
	end
end

-- CoA fix: shows the full debug buffer in a selectable text box (Ctrl+A, Ctrl+C), bypassing chat
-- entirely -- so nothing is lost to the scrollback limit.
function PawnCoAShowDebugLog()
	if not PawnCoADebugLogFrame then
		local f = CreateFrame("Frame", "PawnCoADebugLogFrame", UIParent, (BackdropTemplateMixin and "BackdropTemplate") or nil)
		f:SetSize(760, 520)
		f:SetPoint("CENTER")
		...
		Title:SetText("Pawn CoA -- full debug log (Ctrl+A, Ctrl+C to copy)")

		local CloseButton = CreateFrame("Button", nil, f, "UIPanelCloseButton")
		CloseButton:SetPoint("TOPRIGHT", -5, -5)

		local ScrollFrame = CreateFrame("ScrollFrame", "PawnCoADebugLogScrollFrame", f, "UIPanelScrollFrameTemplate")
		...
		f.EditBox = EditBox
		PawnCoADebugLogFrame = f
	end
	PawnCoADebugLogFrame.EditBox:SetText(table.concat(PawnCoADebugLogBuffer, "\n"))
	PawnCoADebugLogFrame.EditBox:HighlightText()
	PawnCoADebugLogFrame.EditBox:SetFocus()
	PawnCoADebugLogFrame:Show()
end
...
function PawnCommand(Command)
	...
	elseif Command == "debugclear" then
		wipe(PawnCoADebugLogBuffer)
		VgerCore.Message("Pawn CoA: debug log cleared.")
...
-- CoA fix: tooltip scan used as an equippability fallback for custom classes (SunCleric and
-- others) that don't exist in PawnNeverUsableStats. Instead of trying to guess which line is a
-- "weapon/armor type" or "Classes: X" line, we trust the color the CoA client itself already
-- computes: any red "requirement not met" line that does NOT start with "Requires" (level, skill,
-- reputation, arena -- those are temporary, the item can become usable as you progress) is treated
-- as a permanent type/class block.
-- Returns true (usable) or false (not usable).
PawnCoAScanTooltip = nil
function PawnCoAIsClassAllowedByTooltipScan(ItemLink)
	if not ItemLink then return true end
	if not PawnCoAScanTooltip then
		PawnCoAScanTooltip = CreateFrame("GameTooltip", "PawnCoAScanTooltip", nil, "GameTooltipTemplate")
	end
	local Tooltip = PawnCoAScanTooltip
	Tooltip:SetOwner(UIParent, "ANCHOR_NONE")
	Tooltip:ClearLines()
	Tooltip:SetHyperlink(ItemLink)
	for LineIndex = 1, Tooltip:NumLines() do
		local LeftLine = _G["PawnCoAScanTooltipTextLeft" .. LineIndex]
		if LeftLine then
			local Text = LeftLine:GetText()
			-- Ignore temporary requirements ("Requires Level 60", "Requires Skinning (300)",
			-- "Requires Argent Dawn - Friendly" etc.) -- just don't ignore type/class ones.
			if Text and Text ~= "" and strfind(Text, "Requires", 1, true) ~= 1 then
				local r, g, b = LeftLine:GetTextColor()
				-- "Requirement not met" red on this client is roughly (1, 0.1, 0.1).
				if r and r > 0.85 and g and g < 0.25 and b and b < 0.25 then
					return false
				end
			end
		end
...
function PawnGetItemData(ItemLink, LiveMethodName, LiveParam1, LiveParam2)
	if not ItemLink then return end
	...
	if not Item then
		...
		if PawnCommon.Debug then
			...
			-- CoA fix: shows which live-read method was used, for debugging.
			PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] LiveMethodName = " .. tostring(LiveMethodName) .. ", LiveParam1 = " .. tostring(LiveParam1) .. ", LiveParam2 = " .. tostring(LiveParam2))
		end

		-- First the enchanted stats.
		-- CoA fix: servers that rescale items server-side (like CoA/Ascension) only inject the
		-- real, rescaled value when the tooltip is opened via the item's live location (SetBagItem,
		-- SetInventoryItem, SetLootItem). A plain SetHyperlink (what Pawn always did here) shows
		-- the client's base/stale value. When we have the live context (passed in by
		-- PawnUpdateTooltip), we use it; otherwise we fall back to the usual SetHyperlink (works
		-- fine for vendor/trade/mail/auction, which aren't affected by this).
		local UsedLiveTooltipName
		-- CoA fix: true when the stats came from the SetHyperlink fallback due to a ScanEquippedLive
		-- collision (see below) -- i.e. a possibly base/non-rescaled value, used only to avoid
		-- corrupting the real GameTooltip at that instant. See usage further below, near
		-- PawnCacheItem.
		local IsUnreliableFallback = false
		if LiveMethodName == "ReadFromTooltip" then
			-- CoA fix: for an equipped item (SetInventoryItem), a SECOND query against the private
			-- tooltip came back with the base/stale value -- the CoA server seems to only hand out
			-- the correct value on the first query at that instant. Instead of querying twice, we
			-- read directly from the real tooltip that's already open on screen (LiveParam1),
			-- without calling SetInventoryItem again.
			UsedLiveTooltipName = LiveParam1
			PawnFixStupidTooltipFormatting(UsedLiveTooltipName)
			Item.Stats, Item.SocketBonusStats, Item.UnknownLines = PawnGetStatsFromTooltip(UsedLiveTooltipName, true)
		elseif LiveMethodName == "ScanEquippedLive" then
			-- CoA fix: used when we need the stats of an ALREADY EQUIPPED item without the player
			-- hovering it (e.g. comparing against a bag item, or a background scan). There's no
			-- real tooltip already open to reuse here, and a private tooltip doesn't get the
			-- correct CoA value -- so we borrow the real GameTooltip for an instant. CONFIRMED via
			-- diagnostics: without SetOwner, the GameTooltip simply populated NO lines at all (0 of
			-- 19 slots worked in the background scan) -- SetOwner appears to be required, not just
			-- cosmetic.
			if PawnCoAIsScanningEquippedLive or PawnCoAIsBuildingRealTooltip then
				-- CoA fix: either (a) another borrow of the real GameTooltip is already in progress
				-- right now (e.g. the background rescan queue is processing a different slot at
				-- this exact instant), or (b) PawnUpdateTooltip is IN THE MIDDLE of building the
				-- real GameTooltip's content for another reason (e.g. hovering a bag/loot/quest
				-- item) and, to compute whether it's an upgrade, needed to compare against an
				-- equipped slot that was a cache miss. In both cases, stealing the real GameTooltip
				-- right now would overwrite whatever is being (or was just) built into it -- this is
				-- exactly what caused the bag tooltip to go blank/broken when equipping and then
				-- hovering another item of the same type (see PawnGetItemDataForInventorySlot,
				-- called from inside PawnUpdateTooltip("GameTooltip","SetBagItem",...) itself to
				-- compare against equipped slots).
				-- Rather than risk that collision, fall back to the usual SetHyperlink (a base
				-- value, possibly slightly stale for this one instant, but NEVER corrupts the
				-- screen) -- this slot self-corrects on the queue's next pass or the next hover.
				if PawnCommon.Debug then
					PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] ScanEquippedLive(slot=" .. tostring(LiveParam2) .. "): real GameTooltip busy right now (scanning=" .. tostring(PawnCoAIsScanningEquippedLive) .. ", building=" .. tostring(PawnCoAIsBuildingRealTooltip) .. "), falling back to SetHyperlink to avoid a collision")
				end
				...
				UsedLiveTooltipName = PawnPrivateTooltipName
				IsUnreliableFallback = true
				-- CoA fix: also mark this on the Item itself (not just the local IsUnreliableFallback
				-- variable, which only protects PawnCacheItem below) -- other call sites that invoke
				-- PawnGetItemData directly with "ScanEquippedLive" (e.g.
				-- PawnGetItemDataForInventorySlot) also persist the result into their own cache
				-- (PawnCoAEquippedSlotStats) and need this same warning so they don't persist a
				-- base/non-rescaled value there.
				Item.CoAIsUnreliableFallback = true
			else
				local WasShown = GameTooltip:IsShown()
				local SavedOwner, SavedAnchor = GameTooltip:GetOwner()
				local SavedCall, SavedCallN = PawnCoALastGameTooltipCall, PawnCoALastGameTooltipCallN
				PawnCoAIsScanningEquippedLive = true
				GameTooltip:SetOwner(UIParent, "ANCHOR_NONE")
				GameTooltip:ClearLines()
				GameTooltip:SetInventoryItem(LiveParam1, LiveParam2)
				PawnFixStupidTooltipFormatting("GameTooltip")
				Item.Stats, Item.SocketBonusStats, Item.UnknownLines = PawnGetStatsFromTooltip("GameTooltip", true)
				UsedLiveTooltipName = "GameTooltip"
				-- Restores whatever the GameTooltip was showing before our borrow, one frame later.
				-- CoA/WotLK doesn't have C_Timer (that API only exists on much newer clients), so we
				-- use a helper frame with OnUpdate to fire this once on the next frame.
				if not PawnCoADeferFrame then
					PawnCoADeferFrame = CreateFrame("Frame")
				end
				PawnCoADeferFrame:SetScript("OnUpdate", function(self)
					self:SetScript("OnUpdate", nil)
					if SavedCall and SavedCall[1] and GameTooltip[SavedCall[1]] then
						if SavedOwner then
							GameTooltip:SetOwner(SavedOwner, SavedAnchor or "ANCHOR_NONE")
						end
						GameTooltip[SavedCall[1]](GameTooltip, unpack(SavedCall, 2, SavedCallN))
					elseif not WasShown then
						GameTooltip:Hide()
					end
					PawnCoAIsScanningEquippedLive = false
				end)
			end
		elseif LiveMethodName then
			Item.Stats, Item.SocketBonusStats, Item.UnknownLines, Item.PrettyLink = PawnGetStatsFromTooltipWithMethod(PawnPrivateTooltipName, true, LiveMethodName, LiveParam1, LiveParam2)
		else
			Item.Stats, Item.SocketBonusStats, Item.UnknownLines, Item.PrettyLink = PawnGetStatsFromTooltipWithMethod(PawnPrivateTooltipName, true, "SetHyperlink", Item.Link)
		end
		Item.NumLines = (_G[UsedLiveTooltipName or PawnPrivateTooltipName]):NumLines()
		if CachedItem then
		...
		-- CoA fix: this server has 21 fully custom classes and none of the original 13
		-- vanilla/retail classes exist here anymore -- so PawnNeverUsableStats (keyed by the
		-- vanilla numeric ClassID 1-13) is meaningless on this server. Worse than just "unused":
		-- if any custom class happens to report a ClassID that numerically collides with 1-13
		-- (we have no way to verify this for all 21 without logging in as each one), this table
		-- would silently apply the WRONG class's restrictions. Instead, always use the CoA-aware
		-- tooltip color scan (PawnCoAIsClassAllowedByTooltipScan, defined above) -- it reads the
		-- game's own tooltip output directly, so it's correct regardless of class, ClassID, or
		-- how CoA maps custom classes internally.
		Item.CanEquip = PawnCoAIsClassAllowedByTooltipScan(Item.Link) ~= false

		-- CoA fix: items read from the loot window or group roll (Need/Greed/Pass) are never
		-- persisted into Pawn's permanent cache -- the CoA server may still be adjusting the item
		-- at that instant (the same kind of race condition already handled elsewhere). This way,
		-- the next time this item shows up anywhere (bag, equipped, etc.) it's always re-read from
		-- scratch instead of reusing a possibly transient value seen in the loot window.
		-- CoA fix: for the same reason, also don't cache when the stats came from the SetHyperlink
		-- fallback due to a ScanEquippedLive collision (IsUnreliableFallback) -- that value can be
		-- base/non-rescaled. Pawn's cache (PawnItemCache) is indexed only by ItemLink, and if a
		-- wrong value gets cached here, EVERY future read of that same link (bag, equipped, any
		-- context) stays stuck on that wrong value for as long as the tooltip's line count matches
		-- (PawnGetCachedItem/CachedItem.Values, above) -- this is exactly what made the upgrade%
		-- disappear between two specific staves after a few equips: the fallback cached a wrong
		-- value for the item that stayed equipped, and future comparisons against that link kept
		-- reusing the wrong value indefinitely.
		if LiveMethodName ~= "SetLootItem" and LiveMethodName ~= "SetLootRollItem" and not IsUnreliableFallback then
...
-- CoA fix: equipped-slot cache, populated in the background (see PawnCoARescanAllEquippedSlots,
-- called from the UNIT_INVENTORY_CHANGED event and at login). Stores the full Item (already
-- processed, with correct/rescaled stats) keyed by slot number. Used to avoid having to touch the
-- real GameTooltip right when the player is comparing a bag item against what they have equipped
-- -- doing that live, during an active comparison, caused a visible "flicker" in the tooltip (the
-- equipped item's tooltip flashing for an instant instead of the bag item's).
PawnCoAEquippedSlotStats = {}

-- CoA fix: extracts just the numeric item ID from a link, ignoring enchant/gems/suffix/level.
-- Used to compare "is this the same item" in a stable way, since the last field of the link
-- (the player's level at the moment the link was generated) changes as the character levels up.
function PawnCoAGetItemID(Link)
	if not Link then return nil end
	return tonumber(strmatch(Link, "item:(%d+):"))
end

-- Scans ONE of the player's equipped slots in the background and stores the result in the cache.
-- Should only be called outside of an active tooltip (e.g. from an equipment-change event), never
-- while processing a tooltip the player is currently looking at.
-- Returns true if it got a reliable value (or the slot was empty -- nothing to do), false if it
-- collided with the real GameTooltip being busy and needs to be retried later.
function PawnCoARescanEquippedSlot(Slot)
	local ItemLink = GetInventoryItemLink("player", Slot)
	if not ItemLink then
		PawnCoAEquippedSlotStats[Slot] = nil
		return true
	end
	local Item = PawnGetItemData(ItemLink, "ScanEquippedLive", "player", Slot)
	-- CoA fix: same protection as PawnGetItemDataForInventorySlot -- if this call collided with
	-- another call using the real GameTooltip (see Item.CoAIsUnreliableFallback), the value came
	-- back from SetHyperlink (base/non-rescaled). Don't cache that -- leave the slot as it was
	-- (miss or with the old value) to try again later, instead of locking in a wrong value.
	-- IMPORTANT: only signal "retry" when it was actually a collision (Item exists but isn't
	-- reliable). If Item comes back nil (e.g. a shirt/tabard, which has no stats Pawn recognizes),
	-- that will NEVER turn into a success no matter how many times we retry -- without this
	-- distinction, the queue kept retrying the shirt slot 50x every time something was equipped,
	-- just to spam the log.
	local Success = true
	if Item then
		if Item.CoAIsUnreliableFallback then
			Success = false
		else
			PawnCoAEquippedSlotStats[Slot] = Item
		end
	else
		-- CoA fix: even with no recognized stats, store a placeholder with the correct Link (empty
		-- stats). Without this, PawnCoAEquippedSlotStats[Slot] stayed nil forever for items like a
		-- shirt/tabard -- and the periodic poll (every few seconds) saw that as "changed" EVERY
		-- time (empty cache = "Changed = true" in the delta comparison), rescanning (and
		-- re-logging) the same slot forever, even though nothing had actually changed. With the
		-- Link stored here, the ID comparison matches and the slot is only rescanned again once
		-- the item in it actually changes.
		PawnCoAEquippedSlotStats[Slot] = { Link = ItemLink, Stats = {} }
	end
	if PawnCommon.Debug then
		PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnCoARescanEquippedSlot(" .. tostring(Slot) .. ") -> " .. (Item and (Item.CoAIsUnreliableFallback and ("unreliable fallback, retrying later, ID=" .. tostring(PawnCoAGetItemID(Item.Link))) or ("OK, ID=" .. tostring(PawnCoAGetItemID(Item.Link)))) or ("no recognized stats on this item (normal for shirt/tabard etc.), stored the ID only so it won't be rescanned again, ID=" .. tostring(PawnCoAGetItemID(ItemLink)))))
	end
	return Success
end

-- Rescans all of the player's equipped slots in the background -- ONE AT A TIME, spaced out over
-- time (not all at once). CoA fix: we found that scanning all 19 slots in a synchronous loop broke
-- the visible tooltip, because each scan uses the same helper frame to schedule its restoration one
-- frame later -- and scanning the next slot right afterward OVERWROTE the previous slot's scheduled
-- restoration before it ran, leaving the real tooltip in an inconsistent state (only the LAST slot
-- scanned in the loop ever got restored properly).
PawnCoARescanQueue = nil
PawnCoARescanQueueFrame = nil
PawnCoARescanQueueDelay = 0

function PawnCoARescanAllEquippedSlots()
	local FirstSlot = INVSLOT_FIRST_EQUIPPED or 1
	local LastSlot = INVSLOT_LAST_EQUIPPED or 19
	PawnCoARescanQueue = {}
	for Slot = FirstSlot, LastSlot do
		-- CoA fix: only queue the slot if it REALLY changed since the last scan (compares by item
		-- ID, not the full link -- the link's level field changes as the character levels up).
		-- Before this, we rescanned ALL 19 slots on every UNIT_INVENTORY_CHANGED, even to swap a
		-- single ring -- each slot hijacks the real GameTooltip for an instant to read the item,
		-- which was visible as a ~2-3s "flicker" on screen every time you equipped anything, even
		-- for the 18 slots that hadn't changed at all.
		local CurrentLink = GetInventoryItemLink("player", Slot)
		local CachedEntry = PawnCoAEquippedSlotStats[Slot]
		local Changed
		if not CurrentLink then
			Changed = CachedEntry ~= nil -- slot became empty, but the cache still has an old entry
		elseif not CachedEntry then
			Changed = true
		else
			Changed = PawnCoAGetItemID(CachedEntry.Link) ~= PawnCoAGetItemID(CurrentLink)
		end
		if Changed then
			table.insert(PawnCoARescanQueue, Slot)
		end
	end
	if #PawnCoARescanQueue == 0 then
		-- CoA fix: nothing actually changed (e.g. this run came from the periodic poll and nothing
		-- was equipped since the last check) -- don't touch the real GameTooltip at all, and don't
		-- log anything either. This runs every few seconds for the entire play session via the
		-- safety-net poll, so logging a no-op every single time would flood the debug log for no
		-- reason.
		return
	end
	if PawnCommon.Debug then
		PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnCoARescanAllEquippedSlots: starting (" .. #PawnCoARescanQueue .. " slot(s) changed, one at a time)...")
	end
	if not PawnCoARescanQueueFrame then
		PawnCoARescanQueueFrame = CreateFrame("Frame")
	end
	PawnCoARescanQueueDelay = 0
	PawnCoARescanQueueRetryCounts = {}
	PawnCoARescanQueueFrame:SetScript("OnUpdate", function(self, elapsed)
		-- CoA fix: don't touch anything while an active tooltip scan (ours or a real hover) is
		-- happening -- wait our turn, so we don't compete with or overwrite anything.
		if PawnCoAIsScanningEquippedLive then return end
		PawnCoARescanQueueDelay = PawnCoARescanQueueDelay + elapsed
		if PawnCoARescanQueueDelay < 0.1 then return end
		PawnCoARescanQueueDelay = 0
		local Slot = table.remove(PawnCoARescanQueue, 1)
		if not Slot then
			self:SetScript("OnUpdate", nil)
			if PawnCommon.Debug then
				PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnCoARescanAllEquippedSlots: done.")
			end
			return
		end
		local Success = PawnCoARescanEquippedSlot(Slot)
		if not Success then
			-- CoA fix: collided with the real GameTooltip being busy (e.g. the player's mouse
			-- resting on a bag tooltip the whole time, which keeps re-triggering SetBagItem).
			-- Without this, the slot was simply dropped from the queue forever, and its cache stayed
			-- stuck on the old/wrong value until the player manually hovered the equipped item --
			-- exactly the "upgrade%% disappears until I open the character panel" bug. Retry later
			-- (capped, so it never spins forever if something is genuinely stuck).
			local Retries = (PawnCoARescanQueueRetryCounts[Slot] or 0) + 1
			PawnCoARescanQueueRetryCounts[Slot] = Retries
			if Retries <= 50 then -- ~5s of retries (0.1s each) before giving up for good
				table.insert(PawnCoARescanQueue, Slot)
			elseif PawnCommon.Debug then
				PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnCoARescanEquippedSlot(" .. tostring(Slot) .. "): giving up after " .. Retries .. " tries, all collided")
			end
		end
	end)
end
...
function PawnGetItemDataForInventorySlot(Slot, Unenchanted, UnitName)
	...
	local Item
	local NeedsRealUnenchant = false
	local EffectiveLink = ItemLink
	if Unenchanted then
		-- CoA fix: PawnUnenchantItemLink only returns a DIFFERENT link when the item actually has
		-- an enchant/gem to strip -- in that case it becomes a synthetic/hypothetical link. If
		-- nothing changes (the common case, no enchant), it's the same real equipped item.
		local UnenchantedItem = PawnUnenchantItemLink(ItemLink)
		if UnenchantedItem and UnenchantedItem ~= ItemLink then
			NeedsRealUnenchant = true
			EffectiveLink = UnenchantedItem
		end
	end

	if NeedsRealUnenchant then
		-- Synthetic/hypothetical link -- can only be read via SetHyperlink (there's no "live
		-- location" for a hypothetical unenchanted version of the item).
		Item = PawnGetItemData(EffectiveLink)
	elseif UnitName == "player" and PawnCoAEquippedSlotStats[Slot]
		and PawnCoAGetItemID(PawnCoAEquippedSlotStats[Slot].Link) == PawnCoAGetItemID(ItemLink) then
		-- CoA fix: we already have this slot scanned in the background, with the correct/rescaled
		-- value. Compares by item ID (not the full link) because the link's last field changes as
		-- the character levels up, which was making the cache "miss" by mistake even for the same
		-- physical item. Reuse this instead of touching the real GameTooltip right now -- that's
		-- what caused the tooltip flicker during an active comparison.
		if PawnCommon.Debug then
			PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnGetItemDataForInventorySlot(" .. tostring(Slot) .. "): CACHE HIT")
		end
		Item = PawnCoAEquippedSlotStats[Slot]
	else
		-- CoA fix: fallback for when we don't have a cached scan for this slot yet (e.g. just
		-- equipped something and the event hasn't run yet, or it's a unit other than
		-- player/inspect). Can cause a small flicker in this rare case, but guarantees a correct
		-- value.
		if PawnCommon.Debug then
			local Reason
			if UnitName ~= "player" then
				Reason = "UnitName=" .. tostring(UnitName) .. " (not player)"
			elseif not PawnCoAEquippedSlotStats[Slot] then
				Reason = "no cache entry for this slot"
			else
				Reason = "ID mismatch: cache=" .. tostring(PawnCoAGetItemID(PawnCoAEquippedSlotStats[Slot].Link)) .. " current=" .. tostring(PawnCoAGetItemID(ItemLink))
			end
			PawnDebugMessage(VgerCore.Color.Blue .. "[CoA] PawnGetItemDataForInventorySlot(" .. tostring(Slot) .. "): CACHE MISS (" .. Reason .. ")")
		end
		Item = PawnGetItemData(ItemLink, "ScanEquippedLive", UnitName, Slot)
		-- CoA fix: without this, this cache miss NEVER self-corrected -- the only things that wrote
		-- to PawnCoAEquippedSlotStats were the background queue (PawnCoARescanEquippedSlot) or
		-- hovering the item directly on the character panel (SetInventoryItem in PawnUpdateTooltip).
		-- If the queue hadn't reached this slot yet (e.g. right after equipping), EVERY subsequent
		-- hover kept missing again, triggering a new ScanEquippedLive every time -- exactly the bug
		-- pattern (the same bag staff coming back "CACHE MISS" indefinitely until opening the
		-- character panel). Storing it here means the FIRST successful scan already fixes things
		-- for the next hovers.
		-- CoA fix: BUT only store it if the value is reliable (didn't come from the SetHyperlink
		-- collision fallback -- see Item.CoAIsUnreliableFallback). A base/non-rescaled value cached
		-- here stayed PERMANENT (only a real equipment change invalidates this cache by ID), making
		-- upgrade%% comparisons against this slot wrong indefinitely -- this is exactly what made
		-- upgrade%% disappear between two specific staves after a few equips.
		if Item and UnitName == "player" and not Item.CoAIsUnreliableFallback then
			PawnCoAEquippedSlotStats[Slot] = Item
		end
	end
...
-- CoA fix: stores the last call that populated the real GameTooltip (method + parameters), so we
-- can restore its content after using it momentarily to read an equipped item (see
-- PawnCoAScanEquippedSlotLive). Only the real GameTooltip gets the correctly rescaled value from
-- the CoA server -- a private/custom-named tooltip does not, even making the exact same call.
PawnCoALastGameTooltipCall = nil
PawnCoALastGameTooltipCallN = 0
PawnCoAIsScanningEquippedLive = false
-- CoA fix: true while PawnUpdateTooltip is actively building the content of the REAL GameTooltip
-- (bag, loot, roll, quest, equipped, etc.) -- not just during ScanEquippedLive itself. This is
-- needed because building that tooltip can, partway through, need to compare against equipped
-- slots (PawnIsItemAnUpgrade / PawnGetInventoryItemValues); if one of those slots is a cache miss,
-- ScanEquippedLive would try to steal the SAME GameTooltip we're building right now -- clearing/
-- overwriting what had already been written, and then a frame later overwriting it all again on
-- restore. Result: the real tooltip (e.g. a bag item) ended up blank/broken. See usage in
-- PawnGetItemData, ScanEquippedLive branch.
PawnCoAIsBuildingRealTooltip = false
...
function PawnUpdateTooltip(TooltipName, MethodName, Param1, ...)
	...
	-- CoA fix: avoids reentrancy when PawnCoAScanEquippedSlotLive uses the real GameTooltip
	-- under the hood -- those calls shouldn't trigger normal processing again.
	if PawnCoAIsScanningEquippedLive then return end

	if TooltipName == "GameTooltip" and MethodName ~= "ScanEquippedLive" then
		PawnCoALastGameTooltipCall = {MethodName, Param1, ...}
		PawnCoALastGameTooltipCallN = select("#", Param1, ...) + 1
	end
	...
	-- CoA fix: marks that we're building the real GameTooltip right now -- see the comment on the
	-- PawnCoAIsBuildingRealTooltip declaration for why this is needed.
	if TooltipName == "GameTooltip" then
		PawnCoAIsBuildingRealTooltip = true
	end
...
local Item
if ItemLink then
	-- CoA fix: when the original tooltip was opened via the item's live location (bag, equipped,
	-- loot, group roll, quest reward), forward that context to PawnGetItemData instead of letting
	-- it fall back to a plain SetHyperlink -- this is what guarantees getting the server-rescaled
	-- value instead of the client's base value. Vendor/trade/mail/auction still use the usual
	-- SetHyperlink (not affected by this).
	if MethodName == "SetInventoryItem" then
		-- CoA fix: for an equipped item, we do NOT make a second SetInventoryItem call against a
		-- private tooltip -- the CoA server seems to only hand out the correctly rescaled value on
		-- the FIRST query at that instant (the one the game itself just made to draw the tooltip
		-- you see on screen). A second, duplicate call, even milliseconds later, kept coming back
		-- with the wrong base value. Instead of querying twice, we read directly from the tooltip
		-- that's already open on screen (TooltipName), which already has the correct data. Uses
		-- PawnGetItemData (not PawnGetItemDataFromTooltip) to keep Item.Link, CanEquip, and the
		-- rest of normal processing intact.
		Item = PawnGetItemData(ItemLink, "ReadFromTooltip", TooltipName)
		-- CoA fix: piggyback on this read (guaranteed correct, no side effects) to also keep the
		-- equipped-slot cache up to date.
		if Item and Param1 == "player" then
			local InvSlot = ...
			if InvSlot then
				PawnCoAEquippedSlotStats[InvSlot] = Item
			end
		end
	elseif MethodName == "SetBagItem" then
		Item = PawnGetItemData(ItemLink, MethodName, Param1, ...)
	elseif MethodName == "SetLootItem" or MethodName == "SetLootRollItem"
		or MethodName == "SetQuestItem" or MethodName == "SetQuestLogItem" then
		-- CoA fix: same protection as SetInventoryItem above. A second live query -- even to our
		-- own private tooltip -- risks getting the CoA server's base/non-rescaled value instead of
		-- the correct one: the server seems to only hand out the correct value on the FIRST query
		-- at that instant (the one the game itself just made to draw the tooltip you see on
		-- screen). This was confirmed for SetInventoryItem. SetBagItem has been tested extensively
		-- across many sessions and consistently shows correct values via the private-tooltip path,
		-- so it's left alone -- but loot/roll/quest tooltips were never verified, and getting this
		-- wrong here is worse than for a bag item, since Need/Greed/Pass decisions are made under
		-- time pressure during a dungeon run. Reading directly from the already-open real tooltip
		-- removes the risk entirely instead of hoping the server treats these the same as SetBagItem.
		Item = PawnGetItemData(ItemLink, "ReadFromTooltip", TooltipName)
	else
	...
	-- CoA fix: done building the real GameTooltip (if that's what this was) -- clears the way for
	-- ScanEquippedLive to run normally again.
	if TooltipName == "GameTooltip" then
		PawnCoAIsBuildingRealTooltip = false
	end
end
...
function PawnGetStatsFromTooltip(TooltipName, DebugMessages)
	...
	for i = ItemNameLineNumber + 1, Tooltip:NumLines() do
		...
		for Side = 1, 2 do
			...
			-- CoA fix: the "Durability X / Y" line only shows up when the tooltip is opened via
			-- the item's live location (SetBagItem/SetInventoryItem/SetLootItem) -- that's why it
			-- never caused a problem before, when Pawn only used SetHyperlink. Pawn's existing
			-- pattern for ignoring this line doesn't match the CoA format and produces
			-- "Didn't extract a value for 2", so we filter it out here first, before anything else.
			if CurrentParseText and strfind(CurrentParseText, "^Durability %d+ */ *%d+$") then
				CurrentParseText = ""
			end

			-- CoA fix: the client sometimes leaves a stray "|r" (color close tag) at the end of an
			-- entire line, even without an embedded \r\n (e.g. "+17 Intellect|r").
			-- PawnFixStupidTooltipFormatting doesn't reliably catch this case, so we also clean it
			-- up here, at the source, before any separator-based splitting.
			if CurrentParseText and strfind(CurrentParseText, "|r", strlen(CurrentParseText) - 1, true) then
				CurrentParseText = strsub(CurrentParseText, 1, -3)
			end
					...
					while Pos < strlen(CurrentParseText) do
						...
						for _, ThisSeparator in pairs(PawnSeparators) do
							...
							if NextPos then
								...
								-- CoA fix: strips a stray "|r" (color close tag) sometimes left stuck at
								-- the end of a chunk, when the CoA client doesn't close the color before
								-- the separator.
								if strfind(InnerStatLine, "|r", strlen(InnerStatLine) - 1, true) then
									InnerStatLine = strsub(InnerStatLine, 1, -3)
								end
								if ThisLineIsSocketBonus then
						...
						if (Pos > 1) and (not NextPos) then
							...
							-- CoA fix: same stray "|r" cleanup, now for the last chunk of the line
							-- (the most common case: "+7 Critical Strike Rating|r", "+4 Health per 5 sec|r" etc.)
							if strfind(InnerStatLine, "|r", strlen(InnerStatLine) - 1, true) then
								InnerStatLine = strsub(InnerStatLine, 1, -3)
							end
							if ThisLineIsSocketBonus then
		...
		while true do
			...
			if Source == PawnMultipleStatsExtract or Source == PawnSingleStatMultiplier or Source == nil then
				...
				if not ExtractedValue then
					VgerCore.Fail("Didn't extract a value for " .. tostring(Stat) .. " (raw line: \"" .. PawnEscapeString(tostring(ThisString)) .. "\").  Is the translation missing a capture (#)?")
					ExtractedValue = 0
				end
...
function PawnIsItemAnUpgrade(Item, DoNotRescan)
	...
	if InvType == "INVTYPE_WEAPON" or InvType == "INVTYPE_WEAPONMAINHAND" or InvType == "INVTYPE_WEAPONOFFHAND" or InvType == "INVTYPE_SHIELD" or InvType == "INVTYPE_HOLDABLE" or InvType == "INVTYPE_2HWEAPON" or InvType == "INVTYPE_RANGED" then
		...
		local MainWeaponSlot
		if GetInventoryItemLink("player", INVSLOT_MAINHAND) then
			MainWeaponSlot = INVSLOT_MAINHAND
		elseif GetInventoryItemLink("player", INVSLOT_OFFHAND) then
			MainWeaponSlot = INVSLOT_OFFHAND
		end
		if MainWeaponSlot then
			-- CoA fix: reuses PawnGetItemDataForInventorySlot (which already uses the equipped-slot
			-- cache / ScanEquippedLive) instead of a plain PawnGetItemData(MainWeaponLink), which
			-- was reading this item's base/non-rescaled value -- causing confusing debug output and,
			-- more importantly, being yet another source of inconsistent reads of the same item
			-- during a comparison.
			local MainWeapon = PawnGetItemDataForInventorySlot(MainWeaponSlot)
			if MainWeapon then
...
function PawnIsArmorBestTypeForPlayer(Item)
	...
	elseif Class == "DEATHKNIGHT" or Class == "PALADIN" or Class == "WARRIOR" then
		...
		-- CoA fix: custom class (SunCleric and others) with no known armor-type rules. Instead of
		-- hardcoding a fixed hack, trust the tooltip scan: if THIS specific item's armor type is
		-- usable by the current class, count it as the "best type".
		local ScanResult = PawnCoAIsClassAllowedByTooltipScan(Item.Link)
		if ScanResult == false then return false end
		return true
	end
